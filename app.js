...
function renderProducts(){ $("productGrid").innerHTML="";products.filter(p=>p.category===state.category&&(!productSearchTerm||`${p.name} ${p.description||""}`.toLowerCase().includes(productSearchTerm))).forEach(p=>{const c=document.createElement("article");c.className="product-card"+(p.id===state.product.id?" selected":"");c.innerHTML=`<div class="product-visual"><div class="product-shape"></div></div><h3>${p.name}</h3><p>${p.description||""}</p><p><strong>Starting at $${p.base.toFixed(2)}</strong></p>`;c.tabIndex=0;c.setAttribute("role","button");const openProduct=()=>{state.product=p;updateProductUI();renderProducts();changeStep(2)};c.onclick=openProduct;c.onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();openProduct();}};$("productGrid").appendChild(c)})}
function updateProductUI(){ $("selectedProductName").textContent=state.product.name;$("selectedProductPrice").textContent=`Starting at $${state.product.base.toFixed(2)}`;$("designerProductTitle").textContent=state.product.name;$("garmentMockup").className="garment "+(state.product.type||"tee")}
...
document.querySelectorAll("[data-go]").forEach(b=>b.onclick=()=>changeStep(Number(b.dataset.go)));document.querySelectorAll(".step").forEach(s=>s.onclick=()=>changeStep(Number(s.dataset.step)));$("startDesignBtn").onclick=()=>{if(state.product){updateProductUI();changeStep(2);}else{changeStep(1);}};
